async function drawBox(customText?: string) {
  let consoleX: number = process.stdout.rows;
  let consoleY: number = process.stdout.columns;
  if (customText) {
...
function date(t: string | number | Date) {
  let d = new Date(t);
  return (
...
export { color, date, drawBox, clearBox, refreshBox };
